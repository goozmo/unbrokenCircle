<?php
$output = $title = $values = $units = $bgcolor = $icon = $shadow = $custombgcolor = $options = $el_class = '';
...
if ( $bgcolor == "custom" && $custombgcolor != '' ) {
	$custombgcolor = ' style="' . vc_get_css_color( 'box-shadow:0px 4px 0px 0px '.$shadow.'; background-color', $custombgcolor ) . '"';
	$bgcolor = "";
}
if ( $bgcolor != "" ) $bgcolor = " " . $bgcolor;

//if icon
$iconoutput = ( $icon != '' ) ? ' <i class="nicdark_marginright10 '.$icon.'"></i>' : '';
...
foreach ( $graph_lines_data as $line ) {
	...
	$output .= '<div class="vc_single_bar nicdark_bg_grey nicdark_radius nicdark_shadow ' . $bgcolor . '">';
	$output .= '<small class="vc_label title white">'.$iconoutput.' ' . $line['label'] . $unit . '</small>';
